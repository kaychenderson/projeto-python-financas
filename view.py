...
def criar_conta(conta: Conta):
  with Session(engine) as session:
    ...
    if results:
      print("Já existe uma conta no banco!")
      return
...
def desativar_conta(id):
  with Session(engine) as session:
    ...
    conta = session.exec(statement).first()
    if conta.valor > 0:
      raise ValueError("Essa conta ainda possi saldo.")
...
def criar_grafico_por_conta():
  with Session(engine) as session:
    ...
    # bancos = []
    # for i in contas:
    #   bancos.append(i.bancos.value)
    bancos = [i.banco.value for i in contas]
    # total = []
    # for i in contas:
    #   total.append(i.total.value)
...
# criar_conta(conta)
# desativar_conta(1)
# transferir_saldo(2, 3, 15)
# historico = Historico(conta_id = 1, tipos = Tipos.ENTRADA, valor = 5, data = date.today())
...
# plt.bar(['NUBANK', 'SANTANDER'], [15, 11])
# plt.show()